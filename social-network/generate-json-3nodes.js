...
const doc1 = {
  ...
  items: [
    ...
    ...cpp(worker2, 'media-service', 'MediaService'),

    ...cpp(worker2, 'post-storage-service', 'PostStorageService'),
    ...memcached(worker2, 'post-storage-memcached'),
    ...mongodb(worker2, 'post-storage-mongodb'),

    ...cpp(worker1, 'social-graph-service', 'SocialGraphService'),
    ...mongodb(worker1, 'social-graph-mongodb'),
    ...redis(worker1, 'social-graph-redis'),

    ...cpp(worker2, 'text-service', 'TextService'),

    ...cpp(worker2, 'unique-id-service', 'UniqueIdService'),

    ...cpp(worker1, 'url-shorten-service', 'UrlShortenService'),

    ...cpp(worker2, 'user-mention-service', 'UserMentionService'),

    ...cpp(worker1, 'user-service', 'UserService'),
    ...memcached(worker1, 'user-memcached'),
    ...mongodb(worker1, 'user-mongodb'),

    ...cpp(worker2, 'user-timeline-service', 'UserTimelineService'),
    ...mongodb(worker2, 'user-timeline-mongodb'),
    ...redis(worker2, 'user-timeline-redis'),

    ...rabbitmq(worker2, 'write-home-timeline-rabbitmq', 'WRITE-HOME-TIMELINE-RABBITMQ'),
    ...rabbitmq(worker2, 'write-user-timeline-rabbitmq', 'WRITE-USER-TIMELINE-RABBITMQ'),

    ...multi_deployment_service('media-filter-service', [worker1, worker2], {
      containers: [
        {
...
const doc2 = {
  ...
  items: [
    ...
    ...cpp(worker2, 'write-user-timeline-service', 'WriteUserTimelineService'),
  ],
};

fs.writeFileSync('1-3nodes.json', JSON.stringify(doc1, null, 2) + '\n');
fs.writeFileSync('2-3nodes.json', JSON.stringify(doc2, null, 2) + '\n');